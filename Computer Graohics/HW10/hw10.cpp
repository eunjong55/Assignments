...
void get_box_3d(std::vector<GLfloat>& p, std::vector<GLfloat>& normals, GLfloat lx, GLfloat ly, GLfloat lz); // get cube vertices positions
void get_sphere_3d(GLvec& p, std::vector<GLfloat>& normals, GLfloat r, GLint subh, GLint suba);
void get_cone_3d(std::vector<GLfloat>& p, std::vector<GLfloat>& normals, std::vector<size_t>& side_idx, std::vector<size_t>& bottom_idx, GLfloat radius, GLfloat height, GLint n); //to get cone vertices position
void get_cylinder_3d(std::vector<GLfloat>& p, std::vector<GLfloat>& normals, std::vector<size_t>& side_idx, std::vector<size_t>& top_idx, std::vector<size_t>& bottom_idx, GLfloat radius, GLfloat height, GLint n); //to get cylinder vertices position
void get_torus_3d(std::vector<GLfloat>& p, std::vector<GLfloat>& normals, std::vector<std::vector<size_t>>& side_idx, GLfloat r0, GLfloat r1, GLint na, GLint nh); //to get torus vertices position
...
GLvec vtx_pos[7], vtx_clrs[7], vtx_nml[7]; // to store vertex postion and vertex color
GLuint cone_element_buffs[3]; // to use drawElements methods
GLuint cylinder_element_buffs[3]; // to use drawElements methods
GLuint torus_element_buffs[100]; // to use drawElements methods

std::vector<size_t> cone_idx_list[3];
...
void display()
{
    ...
    glUniform1i(4, colorMode); // to set uniform variable where the location is 4
    glUniform1i(5, colorMode);

    if (show_grid) {
        mat4 grid_mat(1.0f);
        glUniform1i(4, 7);
        draw_grid(value_ptr(grid_mat));
        glUniform1i(4, colorMode);
    }
    if (show_axis) {
    ...
    if (show_wireframe) {
        ...
        glUniform1i(4, 3); // to set uniform variable where the location is 4
        if (active_vao == 0) { // cube
            draw_cube(value_ptr(T));
        }
        else if (active_vao == 1) { // sphere
...
void make_cube(GLfloat radius, GLfloat height, GLuint subdivs) {
    get_box_3d(vtx_pos[0], vtx_nml[0], 1, 1, 1); // get cube vertice positions
    ...
    const char * attri_name[3] = { "vPosition", "vNormal", "vColor" };
    GLvec vtx_list[3] = { vtx_pos[0], vtx_nml[0], vtx_clrs[0] };

    glGenBuffers(3, buffs[0]);
    for (int i = 0; i < 3; ++i) bind_buffer(buffs[0][i], vtx_list[i], program, attri_name[i], 3);
}

void make_sphere(GLfloat radius, GLfloat height, GLuint subdivs) {
    get_sphere_3d(vtx_pos[1], vtx_nml[1], radius, height, subdivs); // get sphere vertice positions
    ...
    const char* attri_name[3] = { "vPosition", "vNormal", "vColor" };
    GLvec vtx_list[3] = { vtx_pos[1], vtx_nml[1], vtx_clrs[1] };

    glGenBuffers(3, buffs[1]);
    for (int i = 0; i < 3; ++i) bind_buffer(buffs[1][i], vtx_list[i], program, attri_name[i], 3);

}
void make_cone(GLfloat radius, GLfloat height, GLuint subdivs) {
    get_cone_3d(vtx_pos[2], vtx_nml[2], cone_idx_list[0], cone_idx_list[1], radius, height, subdivs); // get cone vertice positions
    ...
    const char* cone_attri_name[3] = { "vPosition", "vNormal", "vColor" };
    GLvec* cone_vtx_list[3] = { &vtx_pos[2], &vtx_nml[2], &vtx_clrs[2] };
    glGenBuffers(3, buffs[2]);  // generate VBOs
    for (int i = 0; i < 3; ++i) {
...
{
    get_cylinder_3d(vtx_pos[3], vtx_nml[3], cylinder_idx_list[0], cylinder_idx_list[1], cylinder_idx_list[2], radius, height, subdivs);  // get cylinder vertice positions
    ...
    const char* cylinder_attri_name[3] = { "vPosition", "vNormal", "vColor" };
    GLvec* cylinder_vtx_list[3] = { &vtx_pos[3], &vtx_nml[3], &vtx_clrs[3] };
    glGenBuffers(3, buffs[3]);  // generate VBOs
...
{ 
    get_torus_3d(vtx_pos[4], vtx_nml[4], torus_idx_list, r0, r1, na, nh);  // get torus vertice positions
    ...
    const char* torus_attri_name[3] = { "vPosition", "vNormal", "vColor" };
    GLvec* torus_vtx_list[3] = { &vtx_pos[4], &vtx_nml[4], &vtx_clrs[4] };
    glGenBuffers(3, buffs[4]);  // generate VBOs
    for (int i = 0; i < 3; ++i) {
...
void keyboard(unsigned char key, int x, int y)
{
    switch (key)
    {
        ...
        case 'D': colorMode = 0; glutPostRedisplay(); break;
        case 'P': colorMode = 1; glutPostRedisplay();  break;
        case 'G': colorMode = 2; glutPostRedisplay();  break;
    }
}
...
{
    ShaderInfo shaders[] = {
        { GL_VERTEX_SHADER, "hw10.vert" },
        { GL_FRAGMENT_SHADER, "hw10.frag" },
        { GL_NONE, NULL }
    };
...
void get_sphere_3d(GLvec& p, std::vector<GLfloat>& normals, GLfloat r, GLint subh, GLint suba) //to get sphere vertices position
{
...
void get_torus_3d(std::vector<GLfloat>& p, std::vector<GLfloat>& normals, std::vector<std::vector<size_t>>& side_idx, GLfloat r0, GLfloat r1, GLint na, GLint nh) //to get torus vertices position
{
    ...
    for (int i = 0; i <= na; i++) {
        ...
        for (int j = 0; j <= nh; j++) {
            ...
            FPUSH_VTX3_AT(normals, k, x / r1, y / r1, z / r1);
            k++;
        }
    }
    for (int i = 0; i < na; i++) {
